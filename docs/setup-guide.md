...
If what you need is the schema handoff path before real write enablement, see [`/table` schema handoff demo](./table-schema-handoff-demo.md). It walks through both included fixture chains: the baseline path from `examples/feishu-fields-list-response.json` to `examples/feishu-fields-normalized-schema.json` and finally `examples/feishu-fields-mapping-draft.json`, plus the advanced raw-fidelity path from `examples/feishu-fields-list-response-advanced.json` to `examples/feishu-fields-normalized-schema-advanced.json` and `examples/feishu-fields-mapping-draft-advanced.json`. If you only need one quick review block before touching `.env`, use the demo's short advanced excerpt; it compresses `optionCount`, `datetime` formatter drift, and `DuplexLink` relation shape into one quoteable JSON snippet. If you want an even shorter page for review threads or quick env decisions, jump straight to [`/table` schema review page](./table-schema-review-page.md).
